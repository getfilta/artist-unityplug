...
//
//Filta Note: This is basically the same as the Unity skybox shader but with two small render option 
//changes applied. It only exists to fix some in-editor rendering issues.
//
//The problem is that we need to simulate an AR occlusion shader (i.e. for heads, bodies, etc.) in-editor. 
//In the "occluded" areas nothing gets rendered - the buffer from the 
//leftover previous frame gets displayed causing a "hall of mirrors" effect in the occluded areas.
//Because Unity has inexplicably hard-coded skyboxes to have a special render-order that cannot be overridden
//the alternative is to place a sky sphere into the scene with the shader applied.
//For that to work we have to be able to set the render order AND turn zwrite on. So that's what we've done.
...
SubShader {
    ...
    // Cull Off ZWrite False

     Tags { "Queue"="Background-550" "RenderType"="Background"}
    Cull Off ZWrite True
